<template>
  <v-responsive fluid>
    <v-row class="pt-0" align="start" justify="center">
      ...
      <!-- 댓글 보여주기 -->
      <v-col cols="12" v-for="comm in comments" :key="comm.id">
        <CommentComment v-bind="comm" />
      </v-col>
      <!-- 댓글 작성 -->
      <v-col cols="12" >
        <v-card class="mx-2" fixed color="#110B22" 
        
        dark outlined style="border: 1px solid #71d087;">
          <v-list-item>
          ...
          <v-card-actions class="subtitle-2 grey--text text--lighten-5" align="start">
            <v-text-field v-model="loginUserInfo.content" label="댓글" filled rounded dense></v-text-field>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>
  ...
  methods: {
    ...
    PostComments() {
      ...
      CommentApi.PostComments(
        ...
        error => {
          alert("댓글 달기에 오류가 발생했습니다. 잠시후에 다시 시도해주세요 ..");
        }
      );
    },
    getArticleById(id) {
          ...
          for (let c = 0; c < res.data.comments.length; c++) {
            let comment = {
              ...
              pic_name: require("@/assets/images/profile/" +
                ...
                ".png"),
              content: res.data.comments[c][2],
              created_at: res.data.comments[c][3]
            };
            // console.log(comment);
